<script lang="ts">
  ...
  const goalPath = $derived(community ? makeExactCommunityGoalPath(community, event.id) : "")
...
<div data-event={event.id}>
  <Link class="col-2 card2 bg-alt w-full cursor-pointer shadow-md" href={goalPath}>
    {#if censorReason}
