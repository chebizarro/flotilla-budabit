...
describe("reaction publication source contracts", () => {
  ...
  it("awaits same-author delete history for admitted and cached reactions and reports", () => {
    ...
    expect(summary).toContain("cachedReactionDeleteHistoryIncomplete = !result.complete")
    expect(summary).toContain("cachedReportDeleteHistoryIncomplete = !result.complete")
